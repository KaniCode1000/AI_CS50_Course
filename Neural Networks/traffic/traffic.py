...
def load_data(data_dir):
    ...
    for label in range(0,NUM_CATEGORIES):
        tempath = os.path.join(data_dir,str(label))
        allimages = os.listdir(tempath)
        for k in allimages:
            labels.append(label)
            image = cv2.imread(os.path.join(tempath,k))
            resize_image = cv2.resize(image, (IMG_WIDTH, IMG_HEIGHT), interpolation = cv2.INTER_LINEAR)
